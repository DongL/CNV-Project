...
# keyMap_std <- data.frame(pos = c("B4", "B5", "B6",
#                                  "D4", "D5", "D6",
#                                  "F4", "F5", "F6",
#                                  "H4", "H5", "H6",
#                                  "J4", "J5", "J6",
...
# file <- paste(c("IGLL5_grand_table", ".csv"),  collapse = "")
...
```{r grand table, eval = T}
file_list <- list()
fileloc = "/Users/DL/Documents/R project/Memphis - Copy Number Variation/IGLL5 copy typing/IGLL5_table/"
fi <- list.files(fileloc)
for (file in fi){
        file_list[[length(file_list)+1]] <- read.table(paste0(fileloc, file), sep = ",", header = T)
} 

igll5_grand_table <- do.call(rbind, file_list)
hist(igll5_grand_table$RelQ.mean)
hist(igll5_grand_table$Copy.Nbs.round)
write.csv()
