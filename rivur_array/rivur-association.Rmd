...
plt_triple <- function (data = rv, gene = "DEFA1.A3.CN", va) {
        data = data[!is.na(data[va])&!is.na(data[gene])&data[gene] != 'NA'&data[va]!= 'NA', ]
        p = list()
        lapply(seq_along(va), function (i) plt_box(data, gene, va)) -> p[1]
...
```
## Data summary
```{r summary - plot, fig.height = 7, fig.width = 10, eval = F}
par(mfrow = c(2,3))
...
hist(rv$COPY.NBS.ROUND, xlab = "IGLrv.Copy.Nbs", breaks = 12, main = "")
...
va <- c("VR_3L_0101_LABEL", "VR_3L_1301_LABEL")
...
rv$dtHIGHEST_REFLUX_SIDE = as.factor(unclass(rv$HIGHEST_REFLUX_SIDE1301) - unclass(rv$HIGHEST_REFLUX_SIDE0101))
