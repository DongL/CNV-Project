...
#         x = "dtHIGHEST_REFLUX"
#         y = "DEFA1.A3.CN"
#         df = rva1a3_array[!is.na(rva1a3_array[x]) & (rva1a3_array[x] != "NA"), ]
#                 df[x] = factor(df[,x])
#         df = rva1a3_array[!is.na(rva1a3_array["dtHIGHEST_REFLUX"]) & (rva1a3_array["dtHIGHEST_REFLUX"] != "NA"), ]
        oneway_p = oneway.test(as.formula(paste0(y," ~ ",x)), data = df)$p.value
#         print(oneway_p)
        if(oneway_p > 0.2 | oneway_p == "NaN") {
                ggplot(df, aes_string(y = y, x = x)) +
                        geom_point(position = "") +
                        stat_sum(aes(size = ..n..)) +
                        scale_y_continuous(breaks = 3:14) +
...
# rva1a3_array = filter(rva1a3_array, RACE0102 == 1, SEX0101 == "F")
...
hist(rva1a3_array$RACE0102, breaks = 7, xlab = "Race", main = "")
...
```{r cor2, fig.height = 4, fig.width = 14}
...
p = list()
lapply(seq_along(va), function(i) plt_s(df = rva1a3_array[!is.na(rva1a3_array[va[i]]), ], x = va[i], y = "DEFA1.A3.CN")) -> p
do.call(grid.arrange, c(p, list(nrow = 1, ncol =3)))

p = list()
lapply(seq_along(va), function(i) plt_point_s(df = rva1a3_array[!is.na(rva1a3_array[va[i]]), ], x = va[i], y = "DEFA1.A3.CN")) -> p
do.call(grid.arrange, c(p, list(nrow = 1, ncol =3)))
...
p = list()
lapply(seq_along(va), function(i) plt_point_s(df = rva1a3_array[!is.na(rva1a3_array[va[i]]) & (rva1a3_array[va[i]] != "NA"), ], x = va[i], y = "DEFA1.A3.CN")) -> p
do.call(grid.arrange, c(p, list(nrow = 1, ncol =3)))
...
p = list()
lapply(seq_along(va), function(i) plt_s(df = rva1a3_array[!is.na(rva1a3_array[va[i]]) &rva1a3_array[va[i]] != "NA", ], x = va[i], y = "DEFA1.A3.CN")) -> p
do.call(grid.arrange, c(p, list(nrow = 1, ncol =2)))
...
p = list()
lapply(seq_along(va), function(i) plt_s(df = rva1a3_array[!is.na(rva1a3_array[va[i]]) &rva1a3_array[va[i]] != "NA", ], x = va[i], y = "DEFA1.A3.CN")) -> p[1]

lapply(seq_along(va), function(i) plt_point_s(df = rva1a3_array[!is.na(rva1a3_array[va[i]]) &rva1a3_array[va[i]] != "NA", ], x = va[i], y = "DEFA1.A3.CN")) -> p[2]
do.call(grid.arrange, c(p, list(nrow = 1, ncol =2)))
```
